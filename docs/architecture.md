...
Web Worker 是浏览器里的后台线程。十万次模拟放在 Worker 中运行，主页面仍能响应点击和取消操作。
...
页面把快照、样本数和随机种子发送给 Worker。Worker 创建评测实例，再让 `simulation.js` 逐条生成答案并调用 `assessment.js`。普通统计不会把十万条答案全部留在页面内存中。
